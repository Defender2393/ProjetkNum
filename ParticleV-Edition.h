...
class Particle {
    ...
    int TimeSteps = 0;
//Partikelzustand
    vector <array<double, 8>> kinematicContent; //pos 0-2 is for U(x,y,z), pos 3 for d pos 4-6 for posisitions(x,y,z) 7 ist die Cell ID
    //Anzahl Partikel
    int kinematicLine;
...
//U-Datei öffnen und lesen, auslesen der Partikelzahl
    ...
    File.ignore(numeric_limits<streamsize>::max(), '(');        // Springt zum Beginn des Datenblocks '(' und liest alle Partikelgeschwindigkeiten
...
//d-Datei lesen
    ...
    for (int i = 0; i < kinematicLine; i++) {
        ...
        if (SprayorTropfen==2) {
            ...
            for (int i = 0; i < 50; i++) {      // Vorgabe 50 neue Partikel aus particles.txt
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
        ...
        if((FolderNumberStr=="0.18"||FolderNumberStr=="0.38"||FolderNumberStr=="0.58"||FolderNumberStr=="0.78"||FolderNumberStr=="0.98")&&fileName=="Re"){
...
(
   ...
   if (fileName == "Re") {
            if (FolderNumberStr=="0.18"||FolderNumberStr=="0.38"||FolderNumberStr=="0.58"||FolderNumberStr=="0.78"||FolderNumberStr=="0.98") {
...
// Führt einen Zeitschritt der Simulation aus:
    ...
    void increaseTime( vector<array<double,3>> UCell) {
        ...
        for (int i = 0; i < kinematicLine; i++) {
            ...
            if (Spray){
                if (kinematicContent[i][4]==5&&Spray) {     // Partikel hat Zielbereich erreicht → Re = 0
                    RePartikel[i]=0;
                }
                else {
        ...
        if (Spray) {
            for (int i = 0; i < 50; i++) {
                if ((TimeSteps*0.02)==particletxt[i][0]) {  // Fügt neue Spray-Partikel hinzu, wenn Injektionszeit erreicht ist
                    RePartikel.push_back(0);
                    kinematicContent.push_back(array<double, 8>());
    ...
    void Auswertung(string getfolder) {
        ...
        for (int i = 0; i < kinematicLine; i++) {
            if (kinematicContent[i][4]==5) {
                ...
                Auswertung[kinematicContent[i][5]*2]=Auswertung[kinematicContent[i][5]/0.5]+(pow((kinematicContent[i][3]/2),3)*numbers::pi*(4.0/3.0))*998.207;
            }
        }
